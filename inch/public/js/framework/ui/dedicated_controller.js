define(["socket.io", "lib/config", "zepto", "lodash", "lib/any_old_display", "lib/vector_math", "lib/joystick", "lib/window"],
  function(io, config, $, _, AnyOldDisplay, vector_math, joystick, window)
  {
    "use strict";
    ...
    return function(element, width, height, options) {
        ...
        d.keys = {};
        d.input_data = {
            left_stick: { x: 0, y: 0, force: 0.0 },
            right_stick: { x: 0, y: 0, force: 0.0 },
            keys: []
        };
        ...
        var clear_touch = function(control, area_name) { 
            ...
            d.input_data[area_name+'_stick'].x = 0;
            d.input_data[area_name+'_stick'].y = 0;
            d.input_data[area_name+'_stick'].force = 0;
        };
        ...
        var init = function() {
            d.socket = io.connect('/controller', {'force new connection': true});

            d.socket.on('disconnect', function() { d.disconnected(); });
            d.socket.on('connect', function() { d.connected(); });
            d.socket.on('error', function(data) { throw Error(data); });

            if (window.document.hasFocus() && !options.observer) {
                d.socket.emit('unpause');
            }
        ...
        var emit = function() {
            var keys_to_send = [];
            _.each(d.keys, function(value, key) {
                if (value) { 
                    keys_to_send.push(key); 
                }
            });
            d.input_data.keys = keys_to_send;

            if (_.isEqual(d.input_data, d.last_sent)) {
                return;
            }

            d.socket.emit('input', d.input_data);
            console.log(d.input_data.left_stick, d.input_data.right_stick, d.input_data.keys)
            d.last_sent = _.clone(d.input_data, true);
        };
        
        var notifyServerOfInput = function() { setInterval(emit.bind(d), 1000 / 60); }
