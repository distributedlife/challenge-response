...
var onDisconnect = sinon.spy();

var InputModes = [
	{ InputMode: function() { return { getCurrentState: function() { return { 'a': 'a' }; }};}},
	{ InputMode: function() { return { getCurrentState: function() { return { 'b': { 'c': 'c' } }; }};}}
];
var GameMode = 'arcade';
...
describe('desktop socket behaviour', function () {
	  ...
	  jsdom.env({
	    html: html,
	      ...
	      $ = require('zepto-browserify').$;
				...
				Behaviour = require('../src/behaviour').func(defer(window), defer(InputModes), defer(GameMode), defer(serverUrl), defer([onConnect]), defer([onDisconnect]));
	...
	it('should forward the connect event to the ConnectDisconnectBehaviour plugin', function () {
		...
		expect(socket.on.firstCall.args).toEqual(['connect', onConnect]);
	});

	it('should forward the disconnect event to the ConnectDisconnectBehaviour plugin', function () {
		Behaviour.SocketBehaviour(flushPendingAcksFunc).connect(setupFunc, updateFunc);

		expect(socket.on.secondCall.args).toEqual(['disconnect', onDisconnect]);
	});
